...
class Balloon extends React.Component<BalloonProps, BalloonState> {
    ...
    balloonMachine() {
            setInterval(() => {
                const goodBalloons = (): SmallBalloon[] => {
                    if (this.state.balloons.length > this.state.maxBalloons) {
                        let newBalloons = this.state.balloons;
                        newBalloons.shift();
                        return newBalloons ?? [];
                    } else {
                        return this.state.balloons
                    }
                };
                this.setState({
                    balloons: this.state.balloons.filter(
                        (balloon) => balloon.styles.top !== '100%',
                    ),
                });
            this.setState({
                currentRotation: this.state.currentRotation + 1,
                balloons: goodBalloons().concat(
    ...
    balloonStyleGenerator(): React.CSSProperties {
        return {
            ...
            fill: `hsl(${this.state.rotationStep * this.state.currentRotation}, 100%, 80%)`,
    ...
    render() {
        return (
            <div style={balloonWrapperStyle}>
                {this.reversedArrayCopy(this.state.balloons).map((balloon, index) => {
                    return (
                        <svg
                            key={index}
                            width="100%"
